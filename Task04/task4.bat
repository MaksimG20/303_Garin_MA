...
echo "--------------------------------------------------"
sqlite3 movies_rating.db  "create view "T2" as select occupation as 'Occupation', count(occupation) as 'Number' from users group by occupation;" 
...
echo "--------------------------------------------------"
sqlite3 movies_rating.db  "create view T4 as select ratings.user_id as 'userID', ratings.movie_id as 'movieID', ratings.rating as 'Rating', max(ratings.timestamp) as 'Date' from ratings group by ratings.user_id order by timestamp desc;"
...
echo "--------------------------------------------------"
sqlite3 movies_rating.db  "create view T5 as select movies.title as 'Movie', movies.year as 'Year', Rating from movies inner join(select ratings.movie_id, avg(ratings.rating) as 'Rating' from ratings group by ratings.movie_id) ratings on ratings.movie_id = movies.id;"
