...
$course =$_GET['course'];
$st_id = $_GET['id'];
$student = "select * from students where id = '$st_id'";
$FIO = $database->prepare($student);
$FIO->execute();
...
<section>
    ...
    $st_data = $database->prepare($q);
    $st_data->execute();
    $session_info =$st_data->fetchAll(PDO::FETCH_ASSOC);
    //print_r($_GET);
    ?>
    <h1><?=$FIO?></h1>
    ...
    <div class="modal fade" id="create_res" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
        <div class="modal-dialog -centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <?php
                    $data = $database->prepare("select aa.subject as'subject', id as 'id', aa.course as 'course' 
                                        ...
                                        on aa.id = subs.subject_id where subs.subject_id is null");
                    $data ->execute([$dir_id, $course, $st_id, $course]);
